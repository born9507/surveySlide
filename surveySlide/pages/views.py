...
def index(request):
    ...
    if user.is_authenticated:
        ...
        questions = (Question.objects
            ...
            .exclude(choice=None)
            .filter(survey__grade_filter1=grade)
            .filter(survey__grade_filter2=grade)
            .filter(survey__grade_filter3=grade)
            .filter(survey__grade_filter4=grade)
            .filter(survey__gender_filter1=gender)
            .filter(survey__gender_filter2=gender)
        )
...
def surveyCreate(request):
    if request.method == 'POST':
        ...
        gender_filter1=request.POST['gender_filter1']
        gender_filter2=request.POST['gender_filter2']
        grade_filter1=request.POST['grade_filter1']
        grade_filter2=request.POST['grade_filter2']
        grade_filter3=request.POST['grade_filter3']
        grade_filter4=request.POST['grade_filter4']
        survey = Survey(title=title, author=request.user, isCompleted=False, explanation=explanation, gender_filter1=gender_filter1, gender_filter2=gender_filter2, grade_filter1=grade_filter1, grade_filter2=grade_filter2, grade_filter3=grade_filter3, grade_filter4=grade_filter4)
        survey.save()
